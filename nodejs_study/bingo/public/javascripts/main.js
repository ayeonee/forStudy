...
var bingo={
    ...
    init : function(socket){
        ...
        this.is_my_trun=false;
        socket=io();

        socket.on("check_number", function(data){
            self.where_is_it(data.num);
            self.print_msg(data.username+"님이 '"+data.num+"'을 선택했슴다");
        });

        socket.on("game_started", function(data){
            console.log("enter the game_started");
            self.print_msg(data.username+" 님이 게임을 시작했슴다");
            $("#start_button").hide();
        });
        ...
        $("table.bingo-board td").each(function(i){
            ...
            $(this).click(function(){
                if(user_cnt==1){
                    self.print_msg("<알림> 최소 2명부터 게임 가능함다");
                }
                else{
